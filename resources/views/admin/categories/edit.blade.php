    ...
    <div class="space-y-8 max-w-2xl">
        ...
        <div>
            <nav class="flex items-center space-x-2 text-sm text-slate-500 font-medium">
                ...
                <span class="text-slate-400">Edit Category</span>
            </nav>
            <h1 class="text-2xl font-bold text-slate-800 mt-2">Edit Category: {{ $category->nama }}</h1>
        </div>
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="nama" required placeholder="e.g. Surat Keputusan" value="{{ old('nama', $category->nama) }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="color" name="warna" value="{{ old('warna', $category->warna) }}" class="w-full h-11 px-1 py-1 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer">
                        ...
                        <div>
                            ...
                            <input type="text" name="ikon" placeholder="e.g. file-text" value="{{ old('ikon', $category->ikon) }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4" placeholder="Brief description..." class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('deskripsi', $category->deskripsi) }}</textarea>
                ...
                <div class="flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:bg-indigo-700">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
