    ...
    <div class="space-y-8 max-w-2xl">
        ...
        <div>
            <nav class="flex items-center space-x-2 text-sm text-slate-500 font-medium">
                ...
                <span class="text-slate-400">Add Unit</span>
            </nav>
            <h1 class="text-2xl font-bold text-slate-800 mt-2">Add New Unit Kerja</h1>
        </div>
            ...
            <form action="{{ route('admin.unit-kerja.store') }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="kode" required placeholder="e.g. BU" value="{{ old('kode') }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" required placeholder="e.g. Bagian Umum" value="{{ old('nama') }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4" placeholder="Brief description of the division..." class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi') <p class="text-rose-600 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="flex justify-end space-x-3 pt-6 border-t border-slate-100">
                    <a href="{{ route('admin.unit-kerja.index') }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-250 text-slate-700 text-sm font-semibold rounded-xl transition-all">Cancel</a>
